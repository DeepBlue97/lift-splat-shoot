...
def img_transform(img, post_rot, post_tran,
                  ...
                  flip, rotate):
    # adjust image
    img = img.resize(resize_dims)  # img.size: (1600, 900), resize_dims: (327, 184)
    img = img.crop(crop)  # crop: (0, 49, 352, 177)
    if flip:  # True
        img = img.transpose(method=Image.FLIP_LEFT_RIGHT)
    img = img.rotate(rotate)  # rotate: 0.14166049249318124
    # post_rot: tensor([[1., 0.], [0., 1.]]), post_tran: tensor([0., 0.])
    # post-homography transformation
    post_rot *= resize  # resize: 0.20472103667451072
